package net.william278.husktowns.teleport;
...
public class TeleportationHandler {
    ...
    public static void executeTeleport(Player player, TeleportationPoint point) {
        try {
            if (!point.getServer().equals(HuskTowns.getSettings().serverId)) {
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                player.playSound(player.getLocation(), HuskTowns.getSettings().teleportCompleteSound, 1, 1);
    ...
    private static void queueTeleport(Player player, TeleportationPoint point) {
        if (HuskTowns.getSettings().teleportWarmup == 0 || player.hasPermission("husktowns.bypass_teleport_warmup")) {
            executeTeleport(player, point);
        } else {
            final int[] i = {HuskTowns.getSettings().teleportWarmup + 1};
            final Location playerLocation = player.getLocation();
            final double playerHealth = player.getHealth();
            MessageManager.sendMessage(player, "teleportation_warmup_notice",
                    Integer.toString(HuskTowns.getSettings().teleportWarmup));
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (hasMoved(executablePlayer, playerLocation)) {
                        cancel();
                        executablePlayer.playSound(executablePlayer.getLocation(), HuskTowns.getSettings().teleportCancelSound, 1, 1);
                    ...
                    executablePlayer.playSound(executablePlayer.getLocation(), HuskTowns.getSettings().teleportWarmupSound, 1, 1);
                }
